...
def main():
    ...
    if policyId:
        if module.check_mode:
            ...
            policy = obj.get_request('template/policy/vsmart/definition/' + policyId)
            policy_json = policy.json()
            response1 = obj.put_request('template/policy/vsmart/' + policyId, payload=policy_json)
            response2 = obj.post_request('template/policy/vsmart/activate/' + policyId, payload=payload)
